...
+  }
+
+  // 2) Create SSR client — lets us read/refresh session if present
...
+  )
+
+  // 3) Check for session presence (fast path via cookie; fall back to server call)
...
+  }
+
+  // 4) If trying to access a protected page without a session → send to login with next
...
+  }
+
+  // 5) If already authenticated and somehow lands on /login → go to next or dashboard
