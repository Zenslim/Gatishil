...
export default function LoginClient() {
  ...
  async function doLogin(e: React.FormEvent) {
    ...
    try {
      ...
      const looksLikePhone = /^[0-9+()\s-]{6,}$/.test(id);

      const { error } = await supabase.auth.signInWithPassword(
        looksLikePhone ? { phone: id, password } : { email: id, password }
      );
      ...
      if (/invalid login/i.test(m) || /invalid credentials/i.test(m)) {
        setErr(
          'That email/phone and password did not match. If you never set a password, tap “Forgot Password” to create one now, or use the Magic Link option below.'
        );
      } else {
  ...
  // --- Biometric autofill helper (opens platform password manager/passkey UI) ---
      ...
      <div className="w-full max-w-md rounded-2xl border border-white/10 bg-white/5 shadow-2xl p-8">
        ...
        <form onSubmit={doLogin} className="mt-6 space-y-4">
          ...
          <button
            ...
            title="Uses device’s saved password or passkey (FaceID/TouchID/Android biometrics may prompt)."
          >
            🖐️ Use Biometric
          </button>
