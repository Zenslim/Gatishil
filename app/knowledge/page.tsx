...
export default function Page() {
  ...
  return (
    <div style={{ display: 'grid', gap: 16, gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))' }}>
      <Card title={`Knowledge (${rows.length})`}>
        <div style={{ opacity: 0.8, marginBottom: 10 }}>
          Live from <code>/api/knowledge</code>. Add rows in Supabase → <code>public.knowledge</code> and refresh.
        </div>
      ...
      {!loading && !rows.length && !error && (
        <Card title="No rows yet">
          <div style={{ opacity: 0.8 }}>
            Insert rows in Supabase → <code>public.knowledge</code> and refresh.
          </div>
        </Card>
      )}
    </div>
