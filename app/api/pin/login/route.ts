...
const DEBUG_PIN = process.env.NEXT_PUBLIC_DEBUG_PIN === 'true'; // set true temporarily to see real DB errors
...
function err(res: NextResponse, code: number, msg: string, extra?: any) {
  if (DEBUG_PIN && extra) {
    return NextResponse.json({ error: msg, debug: extra }, { status: code });
  }
...
export async function POST(req: NextRequest) {
  try {
    ...
    // Validate env early and loudly
    if (!SUPABASE_URL || !SUPABASE_ANON || !SUPABASE_SERVICE_ROLE_KEY || !PIN_PEPPER) {
      return err(NextResponse.next(), 500, 'Server misconfigured', {
    ...
    // ---------- HEALTH PROBE: can the service-role query anything? ----------
    const health = await svc.from('profiles').select('id').limit(1);
    if (health.error) {
      // Typical if SERVICE_ROLE_KEY is missing/invalid, or table name is wrong
      const e = health.error as PostgrestError;
      return err(NextResponse.next(), 500, 'DB error resolving user', {
        phase: 'health_probe_profiles',
        code: e.code, message: e.message, details: e.details, hint: e.hint,
      });
    }

    // ---------- Resolve profile by email or phone variants ----------
    let profile: { id: string; email: string | null; phone: string | null; phone_e164?: string | null } | null = null;
    ...
    if (!profile?.id) return err(NextResponse.next(), 404, 'User not found');
    ...
    if (pinQ.error) {
      ...
      return err(NextResponse.next(), 500, 'DB error reading PIN', {
        phase: 'read_pin_salt',
        code: e.code, message: e.message, details: e.details, hint: e.hint,
      });
    }
    if (!pinQ.data?.salt) return err(NextResponse.next(), 409, 'No PIN set for this account');

    const derivedPassword = derivePinPassword(pin, userId, pinQ.data.salt, PIN_PEPPER);

    // ---------- Sign in (server-side) ----------
    const authEmail = (profile.email || '').trim();
    if (!authEmail) return err(NextResponse.next(), 500, 'Account email missing');

    const signIn = await supabaseSSR.auth.signInWithPassword({ email: authEmail, password: derivedPassword });
    if (signIn.error) {
      return err(NextResponse.next(), 401, 'Invalid PIN for this account', {
        phase: 'sign_in', message: signIn.error.message,
      });
    }

    return new NextResponse(null, { status: 204 });
  } catch (e: any) {
    return err(NextResponse.next(), 500, 'Unexpected error', { message: e?.message, stack: e?.stack });
  }
}
