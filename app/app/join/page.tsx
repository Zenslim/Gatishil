...
export default function JoinPage() {
  ...
  async function sendPhoneOtp() {
    ...
    try {
      ...
      const res = await fetch('/api/otp/send', { method:'POST', headers:{ 'Content-Type':'application/json' }, body: JSON.stringify({ phone: e164 }) });
  ...
  async function verifyPhoneOtp() {
    ...
    setLoading(true);
    try {
      const res = await fetch('/api/otp/verify', { method:'POST', headers:{ 'Content-Type':'application/json' }, body: JSON.stringify({ phone: e164, code: otp.trim(), name, role }) });
      const data = await res.json();
            ...
            {step === 'collect' && (
              <form onSubmit={(e)=>{e.preventDefault(); sendPhoneOtp();}}>
                ...
                <div style={{display:'flex', gap:8}}>
                  <select value={country.code} onChange={(e)=>{ const next = COUNTRIES.find(c=>c.code===e.target.value)!; setCountry(next); }} style={{...input, maxWidth:200}}>
                    {COUNTRIES.map(c => (<option key={c.code} value={c.code}>{c.flag} {c.name} (+{c.dial})</option>))}
                  </select>
                  <input style={{...input, flex:1}} value={localNumber} onChange={e=>setLocalNumber(e.target.value)} placeholder="98XXXXXXXX" inputMode="numeric" />
                </div>
