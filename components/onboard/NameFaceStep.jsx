...
/**
 * NameFaceStep (guest-friendly)
 * – Keeps UI and photo preview identical.
 * – No Supabase calls, no session dependency.
 * – Stores photo locally (blob URL) and passes data to next step.
 */
export default function NameFaceStep({ t, onBack, onNext }) {
  ...
  const confirmAndSave = async (imgBlob) => {
    ...
    try {
      ...
      setToast("Photo ready.");
    } catch (err) {
      console.error(err);
      setToast("Photo load failed. Try again.");
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <OnboardCardLayout>
      ...
      {toast && (
        <div className="mb-3 rounded-lg bg-emerald-500/15 px-3 py-2 text-sm text-emerald-300">
          {toast}
        </div>
      )}
      ...
      <h2 className="text-2xl font-semibold text-white">
        ...
        <button
          onClick={() =>
            alert(
              t?.nameface?.why ??
                "Faces help real people connect. You control visibility."
            )
          }
          className="underline"
        >
      ...
      <div className="mt-8">
        <button
          ...
          } px-5 py-3 text-black font-semibold disabled:opacity-60`}
        >
          {saving
            ? t?.common?.saving ?? "Saving..."
            : t?.nameface?.cta?.continue ?? "Continue"}
        </button>
      </div>
    </OnboardCardLayout>
  );
